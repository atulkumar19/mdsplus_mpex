function [geo] = get_Proto_geometry2(plotit,newfig,add_skimmer,target_position,add_sleeve)
...
[rcoil,zcoil] = get_coil_cross_sections2;
cmax = max(zcoil,[],2)+1*.0254;  % max and min Z of the coils
cmin = min(zcoil,[],2)-1*.0254;
...
elseif target_position == 2
    target.z = mean([cmax(12),cmin(13)]) + 5*in2m;  % 11.5 + 5"
...
% "Sleeve"
sleeve.r = 0.07315/2;  % ID = 73.15mm
sleeve.z1 = cmin(9)-1*.0254;
sleeve.z2 = sleeve.z1 + 15*in2m; %upstream coil 7, to down 9.  24" 
...
for i=7:13
    ...
    vessel.z(end+1:end+4) = [cmin(i),cmin(i),cmax(i),cmax(i)];
    %vessel.r(end+1:end+4) = [d5     ,d4     ,d4       ,d5       ]/2;
    vessel.r(end+1:end+4) = [d4     ,d5     ,d5       ,d4       ]/2;
end

%vessel.z(end+(1:3)) = [target.z,target.z,5];
%vessel.r(end+(1:3)) = [0,target.r,target.r];
